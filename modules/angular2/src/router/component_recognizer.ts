...
import {Instruction, ResolvedInstruction, ComponentInstruction} from './instruction';
...
export class ComponentRecognizer {
   ...
   */
  recognize(urlParse: Url): Promise<Match>[] {
    var solutions = [];

    this.matchers.forEach((routeRecognizer: RouteRecognizer) => {
  ...
  recognizeAuxiliary(urlParse: Url): Promise<Match>[] {
