...
import {HelloCmp} from './impl/fixture_components';
...
export function main() {
  describe('redirection', () => {
    ...
    xit('should recognize and apply absolute redirects',
        inject([AsyncTestCompleter, Location], (async, location) => {
          compile(tcb)
              .then((rtc) => {rootTC = rtc})
              .then((_) => rtr.config([
    ...
    // TODO: fix and uncomment this
    xit('should recognize and apply relative child redirects',
        inject([AsyncTestCompleter, Location], (async, location) => {
          compile(tcb)
              .then((rtc) => {rootTC = rtc})
              .then((_) => rtr.config([
    ...
    // TODO: fix and uncomment this
    // TODO: need fixture with siblings
    xit('should recognize and apply relative sibling redirects',
        inject([AsyncTestCompleter, Location], (async, location) => {
          compile(tcb)
              .then((rtc) => {rootTC = rtc})
              .then((_) => rtr.config([
                new Redirect({path: '/original', redirectTo: ['../HelloSib']}),
                new Route({path: '/redirected', component: HelloCmp, name: 'Hello'})
              ]))
              .then((_) => rtr.navigateByUrl('/original'))
              .then((_) => {
