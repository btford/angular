...
import {Match, PathMatch, RedirectMatch} from 'angular2/src/router/route_recognizer';
...
function recognize(recognizer: ComponentRecognizer, url: string): Promise<Match[]> {
  var parsedUrl = parser.parse(url);
  return PromiseWrapper.all(recognizer.recognize(parsedUrl));
}

function getComponentType(routeMatch: Match): any {
...
function getParams(routeMatch: Match): any {
  if (routeMatch instanceof PathMatch) {
